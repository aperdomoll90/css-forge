...
describe('HillToggle component', () => {
  const renderComponent = (props = {}) => render(<HillToggle {...props} />)
  ...
  describe('controlled mode', () => {
    const ControlledWrapper = ({ initialChecked = false }: { initialChecked?: boolean }) => {
      ...
      return <HillToggle checked={checked} onToggle={setChecked} />
    }
    ...
    it('should not toggle if parent does not update state', () => {
      const onToggle = jest.fn()
      const { container } = render(<HillToggle checked={false} onToggle={onToggle} />)
      const input = container.querySelector('input[type="checkbox"]') as HTMLInputElement
